...
public class PotionTrigger implements BreakTrigger {
	...
	public void execute(Player player, Block block) {
		player.addPotionEffect(new PotionEffect(potion, dur, amp));
	}

	@Override
	public BreakTrigger register(ConfigurationSection c) {
		try {
			return register(PotionEffectType.getByName(c.getString("effect", "SPEED")), c.getInt("duration", 600),
					c.getInt("amplifier", 1));
		} catch (Exception e) {
